...
{//���ƹ���void *xdma_device_open(const char *mname, struct pci_dev *pdev, int *user_max,int *h2c_channel_max, int *c2h_channel_max)
...
struct pci_device_id pci_ids[] = {
	//vender/device idȷ��ʹ��xilinx 0x10ee, 0x16f2 ,0x16f2 es5341  ����PCI_DEVICE(RTPC_XDMA_VID, RTPC_XDMA_DID)
    { PCI_DEVICE(0x10ee, 0x16f2),.driver_data = (kernel_ulong_t) &vi53xx_info},
	//vender/device idȷ��ʹ��xilinx 0x10ee, 0x16f3 ,0x16f3 es5311
...
//======================================add by ycf 2025.8.13=============================================
	...
	if (xpdev->user_max) {
		 ...
		 */
			...
			/* 使用用户中断，xdma有多个中断，需要MSIX中断支持，才能和AXI DMA中断一起正常工作*/
...
static void remove_one(struct pci_dev *pdev)
{
	...
	if (!xpdev)
		return;
//======================================add by ycf 2025.8.13=============================================
	//vi53xx_dev_clean(xpdev);	//����Ҫxpdev_free(xpdev);��ִ��xpdev_destroy_interfaces(xpdev); �л�ִ��vi53xx_dev_clean(xpdev)
