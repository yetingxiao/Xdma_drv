...
//#define XDMA_BAR1_FD_OFFSET 0x100
...
// 持续发送数据 "0123456789"
void send_data(XUartNs550* UartInstancePtr) {
    char *SendBuffer = "abcdefghijklmnopqrstuvwxyz";
    size_t data_len = strlen(SendBuffer);
    size_t index = 0;
	size_t SendCount=0;
	...
	while(SendCount != data_len){
		
		if(XUartNs550_IsSending(UartInstancePtr))
		{
			usleep(10);
		}
		else
		{
			SendCount += XUartNs550_Send(UartInstancePtr,SendBuffer + SendCount,data_len - SendCount);
		}
	}
...
int main() {
	 ...
	 */
	Status = XUartNs550_Initialize(&UartNs550Instance, UART_DEVICE_ID);
    ...
    printf("Opened XDMA Event devices.\n");
...
/* 		usleep(1);
        ...
        if (pfd.revents & POLLIN) {		   
			XUartNs550_InterruptHandler(&UartNs550Instance);
